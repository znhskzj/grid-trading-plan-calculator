...
### 版本 1.4.1 (2024年8月3日)
- 语言环境设置: 解决了GitHub Actions工作流中的语言环境设置问题，提升了整体稳定性。
- 次要版本更新: 实现了仅对次要版本更新（例如从1.4.0到1.5.0）触发release的逻辑，而不是补丁更新（例如从1.4.0到1.4.1）。
- README中的目录显示: 更新了README文件，以纯文本格式显示目录和文件结构，使其在GitHub上更清晰。

### 版本 1.4.0 (2024年8月3日)
- 重构核心代码： 引入模块化结构，创建 src 目录。
- 引入单元测试框架： 使用 pytest 框架搭建全面的单元测试。
- 升级配置管理： 将配置文件格式从 JSON 转换为 INI。
- 增强错误处理和日志系统： 提升错误处理和日志记录功能。
- 优化项目目录结构： 调整项目文件组织方式。
- 更新构建脚本： 修改构建脚本以适应新的项目布局。
- 提升代码可读性和维护性： 优化代码结构，增强代码的可维护性。
