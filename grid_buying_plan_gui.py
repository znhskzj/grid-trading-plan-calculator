...
def load_config():
    ...
    if not os.path.exists(config_file):
        ...
        logger.info(f"创建了新的配置文件 {config_file}")
...
def calculate_weights(prices, method, max_shares):
    ...
    logger.debug(f"计算得到的权重: {weights}")
...
def calculate_buy_plan(funds, initial_price, stop_loss_price, num_grids=10, allocation_method=0):
    ...
    logger.debug(f"计算购买计划: 资金={funds}, 初始价格={initial_price}, 止损价格={stop_loss_price}, "
                 f"网格数量={num_grids}, 分配方式={allocation_method}")
    ...
    while remaining_funds > min(buy_prices) and loop_count < 1000:  # 添加最大循环次数限制
        ...
        if time.time() - start_time > 5:  # 如果执行时间超过5秒，退出循环
            logger.warning("分配剩余资金时间过长，提前退出循环")
            break

    logger.debug(f"剩余资金分配后的购买数量: {buy_quantities}")
    logger.debug(f"最终剩余资金: {remaining_funds:.2f}")

    buy_plan = [(round(price, 2), int(quantity)) for price, quantity in zip(buy_prices, buy_quantities)]
    logger.debug(f"最终购买计划: {buy_plan}")
...
def run_calculation(event=None, available_funds=None, reserved_funds=0):
    ...
    def calculate():
        ...
        try:
            ...
            logger.debug(f"计算参数: 总资金={total_funds}, 可用资金={available_funds}, 初始价格={initial_price}, "
            ...
            logger.info(f"计算完成，耗时: {end_time - start_time:.2f} 秒")
            logger.debug(f"计算结果: buy_plan={buy_plan}, warning_message={warning_message}")
...
def display_results(
    ...
    def update_gui():
        ...
        if not calculation_in_progress:
            logger.debug("计算已经结束，不更新GUI")
            return
        logger.debug("开始更新GUI")
        try:
            result_text.delete(1.0, tk.END)
            ...
            logger.debug("GUI更新完成")
        except Exception as e:
            logger.exception("显示结果时发生错误")
